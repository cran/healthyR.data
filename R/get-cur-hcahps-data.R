...
#'
#' @description Get the current hcahps data.
#'
#' @details This function will obtain the current HCAPHS data
...
#' will bring back all of the data sets that are in the HCAPS
...
#' \dontrun{
#' current_hosp_data() |>
#'   current_hcahps_data(.data_sets = c("State","National"))
#' }
#'
#' @return
#' Gets the current Complications and Death data from the current hospital data file.
#'
#' @name current_hcahps_data
NULL

#' @export
#' @rdname current_hcahps_data

current_hcahps_data <- function(.data,
    ...
    file_names_vec <- c("hcahps_hospital",
                        "hcahps_national",
                        "hcahps_state")

    asc_list <- l[names(l) %in% file_names_vec]
    names(asc_list)[1] <-  "hcahps_facility"
    ...
    # Return\
    attr(ret, ".list_type") <- "current_hcahps_list"
    class(ret) <- c("current_hcahps_list", class(ret))
