#' Get Current Unplanned Hospital Visits Data.
#'
...
#'
#' @description Get the current Unplanned Hospital Visits data.
#'
#' @details This function will obtain the current Unplanned Hospital Visits data
...
#' bring back all of the data in the Unplanned Hospital Visits data sets that
...
#'
#' @name current_unplanned_hospital_visits_data
NULL

#' @export
#' @rdname current_unplanned_hospital_visits_data

current_unplanned_hospital_vists_data <- function(.data,
    ...
    file_names_vec <- c("unplanned_hospital_visits_hospital",
                        "unplanned_hospital_visits_national",
                        "unplanned_hospital_visits_state")

    asc_list <- l[names(l) %in% file_names_vec]
    names(asc_list)[1] <-  "outpatient_hospital_visits_facility"
    ...
    # Return\
    attr(ret, ".list_type") <- "current_unplanned_hospital_visits_list"
    class(ret) <- c("current_unplanned_hospital_visits_list", class(ret))
